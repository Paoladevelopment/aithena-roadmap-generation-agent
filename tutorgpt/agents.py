...
from tutorgpt.chains import LearningTutorConversationChain, StageAnalyzerChain
from tutorgpt.custom_invoke import CustomAgentExecutor
from tutorgpt.logger import time_logger
from tutorgpt.parsers import TutorConvoOutputParser
from tutorgpt.prompts import LEARNING_TUTOR_TOOLS_PROMPT
from tutorgpt.stages import CONVERSATION_STAGES
from tutorgpt.templates import CustomPromptTemplateForTools
from tutorgpt.tools import get_tools
...
class Tutorgpt(Chain):
    """Controller model for the learning tutor agent."""

    conversation_history: List[str] = []
    conversation_stage_id: str = "1"
    current_conversation_stage: str = CONVERSATION_STAGES.get("1")
    stage_analyzer_chain: StageAnalyzerChain = Field(...)
    tutor_agent_executor: Union[CustomAgentExecutor, None] = Field(...)
    knowledge_base: Union[RunnableSequence, None] = Field(...)
    sales_conversation_utterance_chain: LearningTutorConversationChain = Field(...)
    conversation_stage_dict: Dict = CONVERSATION_STAGES

    model_name: str = "gpt-4o"  

    use_tools: bool = False
    tutor_name: str = "Aithana"
    ...
    async def acall(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        Executes one step of the tutor agent.

        This function overrides the input temporarily with the current state of the conversation,
        generates the agent's utterance using either the tutor agent executor or the tutor conversation utterance chain,
        adds the agent's response to the conversation history, and returns the AI message.

        Parameters
        ----------
        inputs : Dict[str, Any]
            The initial inputs for the tutor agent.

        Returns
        -------
        Dict[str, Any]
            The AI message generated by the tutor agent.
        ...
        if self.use_tools:
            ai_message = await self.tutor_agent_executor.ainvoke(inputs)
    ...
    def _streaming_generator(self):
        ...
        This method is used when the tutor agent needs to take an action before the full LLM output is available.
    ...
    async def _astreaming_generator(self):
        ...
        in-flight of the generation. This is useful in scenarios where the tutor agent wants to take an action
    ...
    def _call(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Generate agent's utterance
        if self.use_tools:
            ai_message = self.tutor_agent_executor.invoke(inputs)
            output = ai_message["output"]
        else:
    ...
    @classmethod
    @time_logger
    def from_llm(cls, llm: ChatLiteLLM, verbose: bool = False, **kwargs) -> "Tutorgpt":
        """
        Class method to initialize the tutorgpt Controller from a given ChatLiteLLM instance.

        This method sets up the stage analyzer chain and tutor conversation utterance chain. It also checks if custom prompts
        ...
        the tutor agent executor and knowledge base to None.

        Parameters
        ----------
        llm : ChatLiteLLM
            The ChatLiteLLM instance to initialize the tutorgpt Controller from.
        ...
        -------
        tutorgpt
            The initialized tutorgpt Controller.
        """
        stage_analyzer_chain = StageAnalyzerChain.from_llm(llm)
        sales_conversation_utterance_chain = LearningTutorConversationChain.from_llm(llm)
        ...
        tutor_agent_executor = None
        knowledge_base = None

        if use_tools:
            shared_learning_repository = kwargs.pop("shared_learning_repository", None)
            tools = get_tools(shared_learning_repository)

            prompt = CustomPromptTemplateForTools(
                template=LEARNING_TUTOR_TOOLS_PROMPT,
            ...
            output_parser = TutorConvoOutputParser(
                ai_prefix=kwargs.get("tutor_name", ""), verbose=verbose
            )

            tutor_agent_with_tools = create_react_agent(
                llm=llm,
                prompt=prompt,
            ...
            tutor_agent_executor = CustomAgentExecutor.from_agent_and_tools(
                agent=tutor_agent_with_tools,
                tools=tools,
                verbose=verbose,
        ...
        return cls(
            ...
            tutor_agent_executor=tutor_agent_executor,
